...
def main():
    ...
    if os.path.isfile(input_path):
        ...
        while cap.isOpened():
            ...
            # image_nparray = np.asarray(frame)
            # print(image)
            ...
            try:
                reading, unit = process_image(frame,
                                            detection_model,
                                            key_point_model,
                                            segmentation_model,
                                            run_path,
            ...
            reading_text = str(reading) + " " + str(unit)
            cv2.putText(frame, reading_text, position, font, font_scale, color, thickness)
            # Display the frame
            cv2.imshow('Gauge Reading', frame)
            ...
            i += 1
            # time.sleep(2)
        # Release the video capture objcect and close the display window
        cap.release()
